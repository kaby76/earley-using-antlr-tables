...
{
    public static ExParser Parser { get; set; }
    public static ErrorListener<IToken> ParserErrorListener { get; set; }
    public static ExLexer Lexer { get; set; }
    ...
    public static string StartSymbol { get; set; } = "p";
    ...
    public static IParseTree Parse(string input)
    {
        ...
        TokenStream = tokens;
        var parser = new ExParser(tokens);
        Parser = parser;
        var listener_lexer = new ErrorListener<int>(false, false, System.Console.Error);
        var listener_parser = new ErrorListener<IToken>(false, false, System.Console.Error);
        lexer.RemoveErrorListeners();
...
//            ParserATNSimulator.trace_atn_sim = true;
...

	{
		...
		var etree = EarleyATN.EarleyAtnRecognizer.ParseToTree(atn, parser.TokenStream, start);
		DateTime eafter = DateTime.Now;
		bool accepted = etree != null;
		if (show_tree)
		{
			if (accepted && !quiet)
			{
				System.Console.WriteLine(etree.ToStringTree(parser));
			}
		}
		Console.WriteLine( (accepted ? "ACCEPT" : "REJECT") + " " + (eafter - ebefore).TotalSeconds);
	}
    }
